Shader "HL/Tablet"
{
	Properties
	{
		...
		[ToggleUI] UI_MasksTexturePacaker("!DRAWER TexturePacker _MaskMap", Int) =  0
		...
		[ToggleUI] UI_TabletHeader("# Tablet Settings", Int) =  0
		_TabletScreenBrightness("Screen Brightness", Range(0,1)) =  1
		...
		[Toggle(UDON_LIGHTMAP_SWITCHING)] _BakedLightmapSwitching("Enable Lightmap Switching", Int) = 0
		[ToggleUI] UI_LightmapSwitchingNote("!NOTE This enables overriding lightmaps using Udon. This is experimental!", Int) = -8
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature_local UDON_LIGHTMAP_SWITCHING
			...
			#if defined(UDON_LIGHTMAP_SWITCHING)
			int _UdonBakeSwitcherEnabled;
			TEXTURE2D(_UdonBakeSwitcherLM);
			TEXTURE2D(_UdonBakeSwitcherDir);
			SAMPLER(sampler_UdonBakeSwitcherLM);
			#endif
			...
			half4 tex2DFastBicubicGeneric(Texture2D targetTex, SamplerState targetTexSampler, half2 uv) {
				#if !defined(PLAT_QUEST) && defined(BICUBIC_LIGHTMAP)
				half width;
				half height;
				unity_Lightmap.GetDimensions(width, height);
				...
				half4 r = g0(fy) * (g0x * SAMPLE_TEXTURE2D(targetTex, targetTexSampler, (half2(px + h0x, py + h0y) * 1.0f / width)) +
				g1x * SAMPLE_TEXTURE2D(targetTex, targetTexSampler, (half2(px + h1x, py + h0y) * 1.0f / width))) +
				g1(fy) * (g0x * SAMPLE_TEXTURE2D(targetTex, targetTexSampler, (half2(px + h0x, py + h1y) * 1.0f / width)) +
				g1x * SAMPLE_TEXTURE2D(targetTex, targetTexSampler, (half2(px + h1x, py + h1y) * 1.0f / width)));
				return r;
				#else
				return SAMPLE_TEXTURE2D(targetTex, targetTexSampler, uv);
				#endif
			};
			...
			void ORLLighting()
			{
				...
				#if defined(UDON_LIGHTMAP_SWITCHING)
				if (_UdonBakeSwitcherEnabled) {
					lightMap = DecodeLightmap(tex2DFastBicubicGeneric(_UdonBakeSwitcherLM, sampler_UdonBakeSwitcherLM, lightmapUV));
				} else {
				...
				half4 lightMapDirection = 0;
				#if defined(UDON_LIGHTMAP_SWITCHING)
				if (_UdonBakeSwitcherEnabled) {
					lightMapDirection = tex2DFastBicubicGeneric(_UdonBakeSwitcherDir, sampler_UdonBakeSwitcherLM, lightmapUV);
				} else {
					lightMapDirection = tex2DFastBicubicGeneric(unity_LightmapInd, samplerunity_Lightmap, lightmapUV);
				}
				#else
				lightMapDirection = tex2DFastBicubicGeneric(unity_LightmapInd, samplerunity_Lightmap, lightmapUV);
				
				// Perform built-in dominant direction logic
				...
				#if defined(UDON_LIGHTMAP_SWITCHING)
				if (_UdonBakeSwitcherEnabled) {
					L0 = DecodeLightmap(tex2DFastBicubicGeneric(_UdonBakeSwitcherLM, sampler_UdonBakeSwitcherLM, lightmapUV));
				} else {
					L0 = tex2DFastBicubicLightmap(lightmapUV, bakedColorTex);
				}
				#else
				L0 = tex2DFastBicubicLightmap(lightmapUV, bakedColorTex);
				#endif
